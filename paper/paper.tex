...
This paper chooses the state variables $q := (h, hu)$ to form a state space, $Q$. As shown in lines XX-XX, this can be expressed in code as a lightweight mutable structure. The choice of single versus double precision for calculations can be specified via a single type alias.
...
This latter problem may be resolved quickly, thanks to Julia's type system.\cite{doi:10.1137/141000671} Rather than having full objects, Julia provides container-like types. Methods have multiple dispatch, which means that a function is resolved to a method by examining all of the types in the signature, rather than simply the `owning' object. This means that operations such addition can be extended to custom types in a streamlined way. This can be seen in lines XX-XX.


Using parametric polymorphism, the spatial domain can be quickly defined as a type alias \verb!QS = Array{Q,1}! This states that spatial variables are implemented as a 1-dimensional array with inner type \verb!Q! as defined above. There is an additional constraint in that there is a ghost cell on either side, so the `true' spatial domain ranges over the indices $[2, n_{cells}+1]$. 
...
This flux function is expressed in lines XX-XX. The shallow water equations enter the numerical code in only one other place, the max wave speed function. The physical speeds of wave propagation are given by the eigenvalues of the flux Jacobian, 
...
will be shown to play an important role in ensuring numerical stability. This is implemented on lines XX-XX.
...
Let us now consider a clean organizing principle for the code. A basic skeleton for running experiments is given on lines XX-XX. The following components are allowed to vary: 
...
The CFL condition is encoded on lines XX-XX. One useful feature is the function chain operator \verb!|>!, which aids in readability. 
...
The viscosity parameter $a$ is taken to be the maximum of the wavespeeds of each neighboring cell. As shown in Figure~\ref{fig:artificial_viscosity}, this exhibits the desired behavior of being large in regions which are discontinuous or evolving quickly, and being small elsewhere. 
